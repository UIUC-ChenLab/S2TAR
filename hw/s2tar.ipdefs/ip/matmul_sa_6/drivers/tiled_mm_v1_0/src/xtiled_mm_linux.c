...
typedef struct {
    ...
    XTiled_mm_uio_map maps[ MAX_UIO_MAPS ];
} XTiled_mm_uio_info;

/***************** Variable Definitions **************************************/
static XTiled_mm_uio_info uio_info;
...
static int uio_info_read_name(XTiled_mm_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XTiled_mm_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XTiled_mm_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XTiled_mm_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XTiled_mm_Initialize(XTiled_mm *InstancePtr, const char* InstanceName) {
	XTiled_mm_uio_info *InfoPtr = &uio_info;
